...
describe('ITSM frontend contracts', () => {
  it('keeps all production endpoint paths explicitly TBD', () => {
    for (const endpoint of Object.values(ITSM_ENDPOINTS)) {
      expect(endpoint.startsWith('TBD ')).toBe(true);
    }
  });
  ...
  it('normalizes an unavailable production adapter as a non-retryable classified error', () => {
    const error = normalizeItsmError(new ItsmAdapterUnavailableError());
    expect(error.kind).toBe('unavailable');
    expect(error.retryable).toBe(false);
    expect(error.message).toContain('never used as a production fallback');
  });
});
