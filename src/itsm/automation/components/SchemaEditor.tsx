...
import { ItsmTabs } from '../../components/ItsmTabs';
...
const schemaTabs = ['Input', 'Output'] as const;
...
function SchemaFields({ title, schema, editable, onChange }: { title: string; schema: AutomationSchema; editable: boolean; onChange: (schema: AutomationSchema) => void }) {
  ...
  return <EuiPanel paddingSize="m" hasBorder data-schema-scope={title.toLowerCase()}>
    <EuiFlexGroup alignItems="center" gutterSize="s" wrap><EuiFlexItem><EuiTitle size="s"><h2>{title} schema</h2></EuiTitle></EuiFlexItem><EuiFlexItem grow={false}><EuiBadge color="hollow">{schema.fields.length} fields</EuiBadge></EuiFlexItem><EuiFlexItem grow={false}><EuiButton size="s" isDisabled={!editable} onClick={addField}>Add field</EuiButton></EuiFlexItem></EuiFlexGroup>
    <EuiSpacer size="m" />
...
export function SchemaEditor({ template, onChanged }: { template: ManagedAutomationTemplate; onChanged: (template: ManagedAutomationTemplate) => void }) {
  ...
  const [activeSchema, setActiveSchema] = useState<string>('Input');
  useEffect(() => { setInputSchema(template.inputSchema); setOutputSchema(template.outputSchema); setActiveSchema('Input'); }, [template]);
  ...
  return <>
    ...
    <EuiSpacer size="m" />
    <EuiPanel paddingSize="none" hasBorder className="itsmAutomationSchemaTabs"><ItsmTabs items={schemaTabs} active={activeSchema} onChange={setActiveSchema} /></EuiPanel>
    <EuiSpacer size="m" />
    {activeSchema === 'Input'
      ? <SchemaFields title="Input" schema={inputSchema} editable={editable} onChange={setInputSchema} />
      : <SchemaFields title="Output" schema={outputSchema} editable={editable} onChange={setOutputSchema} />}
    <EuiSpacer size="m" />
