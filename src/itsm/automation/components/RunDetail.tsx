  ...
  return <>
    <div className="itsmAutomationRunSummary">
      <EuiPanel paddingSize="m" hasBorder>
        <EuiFlexGroup alignItems="center" gutterSize="s" wrap><EuiFlexItem><EuiTitle size="s"><h2>{run.id}</h2></EuiTitle></EuiFlexItem><EuiFlexItem grow={false}><EuiBadge color={runStateColor(run.state)}>{run.state}</EuiBadge></EuiFlexItem><EuiFlexItem grow={false}><EuiBadge color={run.contractMode === 'dry_run' ? 'warning' : 'hollow'}>{run.contractMode}</EuiBadge></EuiFlexItem></EuiFlexGroup>
        <EuiSpacer size="s" />
      ...
      <EuiPanel paddingSize="m" hasBorder className="itsmRunGovernancePanel">
        ...
        <EuiSpacer size="s" />
        <EuiCodeBlock language="json" paddingSize="s" overflowHeight={150}>{JSON.stringify({ failure: run.failure, auditCorrelationId: run.auditCorrelationId }, null, 2)}</EuiCodeBlock>
      </EuiPanel>
    </div>
    <EuiSpacer size="m" />
    <EuiPanel paddingSize="none" hasBorder className="itsmAutomationRunTracePanel">
      <EuiTable responsiveBreakpoint={false} aria-label="Automation run step trace">
