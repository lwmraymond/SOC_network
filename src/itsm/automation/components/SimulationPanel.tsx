...
export function SimulationPanel({ template }: { template: ManagedAutomationTemplate }) {
  ...
  return <div className="itsmSimulationWorkspace">
    <EuiPanel paddingSize="m" hasBorder className="itsmSimulationInputPanel">
      ...
      <EuiSpacer size="m" />
      <EuiTextArea aria-label="Automation simulation sample input" rows={9} value={sampleText} onChange={(event) => setSampleText(event.target.value)} isInvalid={!parsed} />
      ...
      <EuiSpacer size="s" /><EuiButton fill onClick={() => void run()} isLoading={running} isDisabled={!parsed}>Run simulation</EuiButton>
      {error && <><EuiSpacer size="m" /><EuiCallOut title="Simulation unavailable" color="danger">{error}</EuiCallOut></>}
    </EuiPanel>
    <EuiPanel paddingSize="m" hasBorder className="itsmSimulationTracePanel">
      ...
      {result && <>
        ...
        <EuiSpacer size="m" />
        <div className="itsmSimulationTraceTable">
          <EuiTable responsiveBreakpoint={false} aria-label="Automation simulation step trace">
        ...
        <EuiSpacer size="m" /><EuiTitle size="xs"><h3>Expected contract</h3></EuiTitle><EuiCodeBlock language="json" paddingSize="s" overflowHeight={180}>{JSON.stringify({ expectedReceipts: result.expectedReceipts, warnings: result.warnings, sampleInput: result.sampleInput }, null, 2)}</EuiCodeBlock>
      </>}
    </EuiPanel>
  </div>;
}
