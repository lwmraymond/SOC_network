    ...
    {data && <>
      ...
      {section === 'Overview' && <>
        <EuiFlexGroup gutterSize="s" wrap>
          <EuiFlexItem><EuiPanel paddingSize="s" hasBorder><EuiStat title={String(data.templates.length)} description="Templates" titleSize="s" /></EuiPanel></EuiFlexItem>
          <EuiFlexItem><EuiPanel paddingSize="s" hasBorder><EuiStat title={String(data.templates.filter((item) => item.status === 'published').length)} description="Published templates" titleSize="s" /></EuiPanel></EuiFlexItem>
          <EuiFlexItem><EuiPanel paddingSize="s" hasBorder><EuiStat title={String(data.rules.length)} description="Rules" titleSize="s" /></EuiPanel></EuiFlexItem>
          <EuiFlexItem><EuiPanel paddingSize="s" hasBorder><EuiStat title={String(data.runs.filter((item) => item.state === 'failed' || item.state === 'partial').length)} description="Failed / partial runs" titleSize="s" /></EuiPanel></EuiFlexItem>
        </EuiFlexGroup>
        <EuiSpacer size="m" />
        <div className="itsmAutomationOverviewGrid">
          <EuiPanel paddingSize="m" hasBorder><EuiTitle size="s"><h2>Template lifecycle</h2></EuiTitle><EuiSpacer size="s" />{data.templates.slice(0, 6).map((template) => <div className="itsmCapabilityRow" key={template.id}><span><strong>{template.name}</strong><small>{template.category} · {template.owner} · {template.publishedVersionId ?? 'not published'}</small></span><EuiBadge color={templateStatusColor(template.status)}>{template.status}</EuiBadge></div>)}<EuiSpacer /><EuiButtonEmpty href="/itsm/automation/templates">Open Template Library</EuiButtonEmpty></EuiPanel>
          <EuiPanel paddingSize="m" hasBorder><EuiTitle size="s"><h2>Recent execution health</h2></EuiTitle><EuiSpacer size="s" />{data.runs.slice(0, 6).map((run) => <div className="itsmCapabilityRow" key={run.id}><span><strong>{run.id}</strong><small>{run.templateName ?? run.templateId ?? run.ruleId} · {run.triggerSource ?? 'TBD'} · {formatDuration(run.durationMs)}</small></span><EuiBadge color={runStateColor(run.state)}>{run.state}</EuiBadge></div>)}<EuiSpacer /><EuiButtonEmpty onClick={() => setSection('Execution runs')}>Open execution runs</EuiButtonEmpty></EuiPanel>
          <EuiPanel paddingSize="m" hasBorder><EuiTitle size="s"><h2>Runtime governance</h2></EuiTitle><EuiSpacer size="s" /><EuiCallOut title="Execution engine settings">Queue, concurrency, timeout, retry, dead-letter, idempotency, approval, compensation, retention and audit-correlation settings are managed on a dedicated route.</EuiCallOut><EuiSpacer /><EuiButtonEmpty href="/itsm/automation/runtime">Open Runtime Settings</EuiButtonEmpty></EuiPanel>
        </div>
      </>}
      ...
      {section === 'Execution runs' && <EuiPanel paddingSize="none" hasBorder>
        <EuiTable aria-label="Automation execution runs">
