...
export function P23CopilotWorkspace({ fixture }: { fixture: PrototypePageFixture }) {
  ...
  const messages = useMemo(() => fixture.timeline.slice(0, 5), [fixture.timeline]);
    ...
    <EuiFlexGroup gutterSize="m" alignItems="stretch" responsive={false}>
      ...
      <EuiFlexItem grow={6}><div data-visual-region="copilot-conversation-workbench">
        ...
        <EuiPanel paddingSize="m" hasBorder><EuiFlexGroup alignItems="center"><EuiFlexItem><EuiTitle size="s"><h2>{selected.title}</h2></EuiTitle><p>{selected.linked} · session {selected.id}</p></EuiFlexItem><EuiFlexItem grow={false}><EuiBadge color="warning">Evidence conflict</EuiBadge></EuiFlexItem></EuiFlexGroup><EuiSpacer size="m" />
          <div className="conversationTranscript">{messages.map((item, index) => <article key={`${item.time}-${index}`} className={index % 2 ? 'assistant' : 'user'}><header><strong>{index % 2 ? 'Copilot' : 'Analyst'}</strong><time>{item.time}</time></header><p>{index % 2 ? `${item.detail} Evidence remains cited and the conflicting telemetry source is not silently resolved.` : item.title}</p>{index % 2 && <div>{citations.slice(0, 2 + (index % 2)).map((citation) => <EuiBadge key={citation.id} color={citation.conflict ? 'danger' : 'hollow'}>{citation.id} · {citation.source}</EuiBadge>)}</div>}</article>)}</div>
      ...
      <EuiFlexItem grow={3}><EuiPanel paddingSize="m" hasBorder data-visual-region="copilot-evidence-tool-context">
        ...
        <EuiTitle size="xs"><h3>Citations</h3></EuiTitle>{citations.map((item) => <div className="copilotCitationRow" key={item.id}><strong>{item.source}</strong><span>{item.age} · {item.coverage}</span><EuiBadge color={item.conflict ? 'danger' : 'success'}>{item.conflict ? 'Conflict' : 'Fresh'}</EuiBadge></div>)}
