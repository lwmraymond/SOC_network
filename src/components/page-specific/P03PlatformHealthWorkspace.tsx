      ...
      <EuiFlexGroup gutterSize="m" alignItems="stretch" responsive={false}>
        <EuiFlexItem grow={4}>
          <EuiPanel paddingSize="m" hasBorder data-visual-region="active-degradation-chain">
            ...
            <EuiSpacer size="m" />
            <div className="p03DependencyPath">{dependencyStages.map((stage, index) => <EuiPanel className="p03DependencyStage" key={stage.label} paddingSize="s" hasBorder color={index === 2 ? 'subdued' : 'plain'}><EuiText size="xs" color="subdued"><p>{stage.label}</p></EuiText><strong>{stage.detail}</strong><EuiSpacer size="s" /><EuiHealth color={stage.color}>{stage.state}</EuiHealth></EuiPanel>)}</div>
            <EuiSpacer size="m" />
