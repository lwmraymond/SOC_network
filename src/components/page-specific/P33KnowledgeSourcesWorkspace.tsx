...
type Source = { id: string; name: string; type: string; location: string; owner: string; classification: string; state: string; freshness: string; documents: number; failures: number; acl: string; citation: number; quality: string; lastSync: string };
...
export function P33KnowledgeSourcesWorkspace({ fixture }: { fixture: PrototypePageFixture }) {
  ...
  const selected = visible.find((item) => item.id === selectedId) ?? visible[0] ?? sources[0];
  ...
  return <div className="pageComposition page-p33 differentiatedPage p33Knowledge" data-page-specific-composition="P33-source-sync-acl-citation">
    ...
    <EuiSpacer size="m" /><div className="p33Workspace">
      <EuiPanel paddingSize="m" hasBorder className="p33Grid"><EuiTitle size="s"><h2>Knowledge source library</h2></EuiTitle><table><thead><tr><th>Source</th><th>Type</th><th>Sync</th><th>Freshness</th><th>Documents</th><th>ACL</th><th>Citation</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td><EuiButtonEmpty size="xs" onClick={() => setSelectedId(item.id)}>{item.id}</EuiButtonEmpty><small>{item.name}</small></td><td>{item.type}</td><td><EuiBadge color={item.state === 'Active' ? 'success' : item.state === 'Failed' ? 'danger' : 'warning'}>{item.state}</EuiBadge></td><td>{item.freshness}</td><td>{item.documents}</td><td>{item.acl}</td><td>{item.citation}%</td></tr>)}</tbody></table></EuiPanel>
