...
export function P04CaseInvestigationWorkspace({ fixture }: { fixture: PrototypePageFixture }) {
  ...
  return (
    <div className="pageComposition page-p04 differentiatedPage" data-page-specific-composition="P04-case-investigation">
      ...
      <EuiFlexGroup gutterSize="m" alignItems="stretch" responsive={false}>
        ...
        <EuiFlexItem grow={5}>
          <EuiPanel paddingSize="m" hasBorder data-visual-region="selected-case-investigation">
            <EuiFlexGroup justifyContent="spaceBetween" alignItems="flexStart" responsive={false}><EuiFlexItem><EuiBadge color="danger">{selectedCase.severity} · Risk {selectedCase.risk}</EuiBadge><EuiSpacer size="s" /><EuiTitle size="m"><h2>{selectedCase.title}</h2></EuiTitle><EuiText size="s" color="subdued"><p>{selectedCase.id} · {selectedCase.classification} · owner {String(selectedCase.owner)}</p></EuiText></EuiFlexItem><EuiFlexItem grow={false}><EuiButtonEmpty onClick={() => setPreviewOpen(true)}>Preview case facts</EuiButtonEmpty><EuiButton fill>Open full case</EuiButton></EuiFlexItem></EuiFlexGroup>
      ...
      {previewOpen && <EuiFlyout ownFocus size="m" onClose={() => setPreviewOpen(false)} aria-labelledby="p04-case-preview-title"><EuiFlyoutHeader hasBorder><EuiTitle size="m"><h2 id="p04-case-preview-title">{selectedCase.title}</h2></EuiTitle><EuiText size="s" color="subdued"><p>{selectedCase.id} · {selectedCase.classification}</p></EuiText></EuiFlyoutHeader><EuiFlyoutBody><EuiCallOut title="Case preview" color="primary">Preview supports triage without changing case state. Full investigation remains in the selected case workspace.</EuiCallOut><EuiSpacer size="m" /><EuiFlexGroup gutterSize="m" wrap><EuiFlexItem><EuiPanel paddingSize="m" hasBorder><EuiText size="xs" color="subdued"><p>Key entities</p></EuiText><EuiTitle size="s"><h3>{selectedCase.entities}</h3></EuiTitle></EuiPanel></EuiFlexItem><EuiFlexItem><EuiPanel paddingSize="m" hasBorder><EuiText size="xs" color="subdued"><p>MITRE</p></EuiText><EuiTitle size="s"><h3>{selectedCase.mitre}</h3></EuiTitle></EuiPanel></EuiFlexItem><EuiFlexItem><EuiPanel paddingSize="m" hasBorder><EuiText size="xs" color="subdued"><p>Response</p></EuiText><EuiTitle size="s"><h3>{selectedCase.response}</h3></EuiTitle></EuiPanel></EuiFlexItem></EuiFlexGroup><EuiSpacer size="m" /><EuiTitle size="xs"><h3>Latest activity</h3></EuiTitle><p>{selectedCase.activity}</p></EuiFlyoutBody><EuiFlyoutFooter><EuiButtonEmpty onClick={() => setPreviewOpen(false)}>Close preview</EuiButtonEmpty><EuiButton fill onClick={() => setPreviewOpen(false)}>Continue investigation</EuiButton></EuiFlyoutFooter></EuiFlyout>}
    </div>
  );
}
