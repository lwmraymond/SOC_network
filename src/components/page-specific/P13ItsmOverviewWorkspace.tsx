...
import { EuiBadge, EuiButton, EuiButtonEmpty, EuiCallOut, EuiFieldSearch, EuiFlexGroup, EuiFlexItem, EuiModal, EuiModalBody, EuiModalFooter, EuiModalHeader, EuiModalHeaderTitle, EuiPanel, EuiProgress, EuiSelect, EuiSpacer, EuiTitle } from '@elastic/eui';
...
export function P13ItsmOverviewWorkspace({ fixture }: { fixture: PrototypePageFixture }) {
  ...
  const create = (kind: string) => { setReceipt(`${kind} submission accepted in prototype mode. No production ITSM object was created.`); setCreateOpen(false); };
  return <div className="pageComposition page-p13 differentiatedPage" data-page-specific-composition="P13-service-operations-command">
    ...
    {createOpen&&<EuiModal onClose={()=>setCreateOpen(false)} aria-labelledby="p13-create-title"><EuiModalHeader><EuiModalHeaderTitle id="p13-create-title">Create typed work item</EuiModalHeaderTitle></EuiModalHeader><EuiModalBody><EuiCallOut title="Choose the correct workflow">Incident, Request, Change and Problem use different forms, state machines and ownership.</EuiCallOut><EuiSpacer/><EuiFlexGroup gutterSize="s">{['Incident','Request','Change','Problem'].map((kind)=><EuiFlexItem key={kind}><EuiButton fullWidth onClick={()=>create(kind)}>{kind}</EuiButton></EuiFlexItem>)}</EuiFlexGroup></EuiModalBody><EuiModalFooter><EuiButtonEmpty onClick={()=>setCreateOpen(false)}>Cancel</EuiButtonEmpty></EuiModalFooter></EuiModal>}
  </div>;
}
